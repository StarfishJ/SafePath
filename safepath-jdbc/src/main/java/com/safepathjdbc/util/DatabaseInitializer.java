            ...
            try (Connection conn = ConnectionManager.getConnection()) {
                ...
                try (java.sql.Statement stmt = conn.createStatement()) {
                    try {
                        ...
                        // Check error code - 42104 means table not found in H2
                        int errorCode = e.getErrorCode();
                        String errorMsg = e.getMessage();
                        
                        // H2 error code 42104 = table not found
                        if (errorCode == 42104) {
                            tableExists = false;
                            System.out.println("DatabaseInitializer: ✗ users table does NOT exist (error code 42104)");
                        } else if (errorMsg != null && errorMsg.contains("not found")) {
                            tableExists = false;
                            System.out.println("DatabaseInitializer: ✗ users table does NOT exist (message contains 'not found')");
                        } else if (errorMsg != null && errorMsg.contains("empty")) {
                            tableExists = false;
                            System.out.println("DatabaseInitializer: ✗ Database is empty");
                        } else {
